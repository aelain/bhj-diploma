...
class TransactionsPage {
  ...
  registerEvents() {
    ...
    this.element.addEventListener("click", event => {
      event.preventDefault();
      const btnTransactionRemove = event.target.closest('.transaction__remove');

      if (btnTransactionRemove) {
        const id = { id: btnTransactionRemove.dataset.id };
  ...
  removeAccount() {
    if (this.lastOptions) {
      const consent = confirm('Вы действительно хотите удалить счёт?');
  ...
  removeTransaction(id) {
    const consent = confirm('Вы действительно хотите удалить эту транзакцию?');
  ...
  formatDate(date) {
    ...
    return `${day} ${month[monthIndex]} ${year} г. в ${hours}:${minutes}`;
  }
  ...
  getTransactionHTML(item) {
    ...
    transactionElement.innerHTML = `<div class="col-md-7 transaction__details">
